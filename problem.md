...
点搜索出现常见搜索（有余力就添加）
后端新增-热门搜索功能
- 新增SearchLog模型用于记录用户搜索词
- 修改/api/calligraphy/search接口，搜索时自动记录关键词
- 新增/api/calligraphy/hot-keywords接口，返回最近7天热门搜索词
- 更新init_db.py添加SearchLog模型导入
前端新增-检索页热门搜索功能
- search.html添加热门搜索标签展示区域
- search.js添加loadHotKeywords函数获取并渲染热门搜索词
- 点击热门标签自动填入搜索框并执行搜索
- 无搜索记录时自动隐藏热门搜索区域
优化-检索页面样式美化与搜索候选功能
- 重构search-layout.css：优化页面布局、搜索框样式，添加渐变和阴影效果
- 重构search-cards.css：美化作品卡片和单字卡片，添加悬停动画效果
- 修改search.html：添加搜索候选下拉框HTML结构
- 扩展search.js：实现作品名和作者的正则匹配搜索候选功能，显示top-8结果
- 新增防抖动输入处理和点击外部关闭下拉框交互
查漏补缺-检索页显示问题修复
- 新增STYLE_MAP映射表，修复书法风格显示英文缩写(cao/kai/xing等)改为中文(草书/楷书/行书等)
- 修复作品卡片、弹窗、搜索候选中书法风格显示问题
- 重构作品卡片布局：改为垂直卡片式（图片在上、信息在中、操作栏在下）
- 采用grid网格布局替代原有列表布局
- 添加card-footer底部操作栏，朝代信息在左、查看详情按钮在右
- 修复信息区域高度过大问题，使用!important强制重置margin/padding
- 恢复作品描述显示，限制最多2行自动省略
